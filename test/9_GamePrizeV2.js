...
describe("Game Prize contract", function () {
  async function deployTokenFixture() {
    ...
    await GamePrize.connect(owner).setVaultOperator(operator.address, true);
    ...
    await GamePrize.connect(owner).setDripAmount(
      MockERC20.address,
  ...
  describe("Deployment", function () {
    ...
    it("Should have a correct GamePrize operator", async function () {
      ...
      expect(await GamePrize.vaultOperator(operator.address)).to.equal(true);
    });
  ...
  describe("Airdrop", function () {
    it("Should drip correctly", async function () {
      ...
      await expect(
        GamePrize.connect(operator).drip(MockERC20.address, player1.address, ethers.utils.parseEther("1.0"))
      )
  ...
  describe("Withdraw", function () {
    it("Should drip and withdraw correctly", async function () {
      const { MockERC20, GamePrize, owner, operator, player1, player2 } =
        await loadFixture(deployTokenFixture);
      await GamePrize.connect(operator).drip(MockERC20.address, owner.address, ethers.utils.parseEther("1.0"));
